...
{
    protected
        $configServer,          //apollo服务端地址

        $appId,                 //apollo配置项目的appid

        $namespaces,            //apollo 域名空间

        $cluster = 'default',

        $clientIp = '127.0.0.1',//绑定IP做灰度发布用

        $notifications = [],

        $pullTimeout = 10,      //获取某个namespace配置的请求超时时间

        $intervalTimeout = 60;  //每次请求获取apollo配置变更时的超时时间
    ...
    private function modifyEnv(array $data)
    {
        ...
        $contentArray = [
            'APOLLO_APP_ID' => $this->appId,
            'APOLLO_CLUSTER' => $this->cluster,
            'APOLLO_NAMESPACES' => $this->namespaces,
            'APOLLO_CONFIG_SERVER' => $this->configServer
        ];
        foreach ($data as $key => $value){
            $contentArray[$key] = $key . '=' . $value;
        }
     ...
     * @param $locate 语言
     */
    private function modifyLang($content,$locate)
    {
        $langPath = resource_path("lang/{$locate}/");
        if(!File::exists($langPath)){
            File::makeDirectory($langPath,0777,true);
        }

        $langFile = $langPath.'lang.yml';
    ...
    public function pullConfig($namespaceName) {
        ...
        if ($httpCode == 200) {
            ...
            if(isset($result['configurations'])){
...
//        }
        ...
        foreach ($request_list as $namespaceName => $req) {
            ...
            if ($code == 200) {
                ...
                if(Str::substr($namespaceName,0,4) == 'Lang' ){
                    // lang
                    if(isset($result['configurations']['content'])){
                        $namespaceNameArr = explode("_",$namespaceName);
                        $locale = count($namespaceNameArr) == 2 ? end($namespaceNameArr) : "en";
                        $this->modifyLang($result['configurations']['content'],$locale);
                    }
                }else{
                    // config
                    if(isset($result['configurations'])){
    ...
    /**
     * @param \Closure $callback 监听到配置变更时的回调处理
     * @return mixed
     */
